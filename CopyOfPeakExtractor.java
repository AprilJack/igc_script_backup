...
public class CopyOfPeakExtractor {
	...
	public static void main(String[] args) {
		...
		try {
			...
			long segLengths = 0;
			String lastEnd = null;
			bed.println("track name="+args[1]+" description=\"bedGraph peaks with threshold "+args[2]+"\" useScore=1");
			while(s.hasNextLine())
			{
				String line = s.nextLine();
					...
					else if(in_peak &&((value < threshold)|| (lastEnd != null &&lastEnd.compareTo(split[1])!=0))) //fell below or missing values
					{
						if(seq.length() > 30)
						{
							...
							for(int i = 0; i < 3; i++)
							{
								...
								for(int j = 0; j < translated.length; j++)
								{
									if(translated[j].length() > 9 && translated[j].indexOf('?')==-1 && chrs.get(chrName).length > (seqStart+i+seqThusFar+translated[j].length()*3))
									{
										threeframe.println(">Peak "+count+" FWD: "+forward+" frame "+(i+1)+" segment "+j);						
										threeframe.println(translated[j]);
										String strand = "+"; if(!forward) strand = "-";
										bed.println(chrName+"\t"+(seqStart+i+seqThusFar-1)+"\t"+(seqStart+i+seqThusFar+translated[j].length()*3-1)+"\t"+
												"Peak"+count+"_frame"+(i+1)+"_seg"+j+"\t"+(int)(Math.min(1000,500+intensity/seq.length()))+"\t"+strand);
										seg_count++;
										segLengths+=translated[j].length()*3;
									}
									if(j ==0)
										seqThusFar+=(translated[j].length()*3);
									else seqThusFar+=(3+translated[j].length()*3);
	...
	private static String getSequence(String chr, int start,
			int stop) {
		char[] chars = new char[(int) (stop-start+1)];
	...
	private static String getFrameTranslated(String seq, int frame)
	{
		...
		{
			String shift = revComplement(seq).substring(frame-3);
			String trans = seqToAA(shift);
			return trans;
		}
	}
