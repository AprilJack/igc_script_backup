...
if [[ $# -lt 1 ]]
then
	echo "Usage: startDemultiplexer.sh DirectoryContainingNewlySequencedRunFolders email1 email2 email3 ..."
	echo "Runs continuously so make sure you have screen activated! Errors? Contact Salk IGC"
	here=/gpfs/smb/illumina/runfolders 
	email='nhah@salk.edu igc@salk.edu ouyang@salk.edu' #if more than one email, do this --> lliu@salk.edu maxshok@gmail.com another@salk.edu another@salk.edu ...
...
#backup=/boll/cellar/illumina/fastq
...
while true; do
	runDirs=$(find $here -maxdepth 3 -name RTAComplete.txt -printf '%h\n') #Find RTAComplete.txt
